...
if __name__ == '__main__':
    ...
    # good = [row for row in dataset if row[-1] == 1]
    # bad = [row for row in dataset if row[-1] == 0]

    # train_set = good[:int(x/100 * len(good))] + bad[:int(x/100 * len(bad))]
    ...
    importance = list(classifier1.feature_importances_)
    least_importanc = importance.index(min(importance))
    ...
    for row in dataset:
        ...
        for i in range(len(row)):
            if i != least_importanc:
                new_row.append(row[i])
        dataset_new.append(new_row)

    train_set_new = dataset_new[int(x / 100 * len(dataset_new)):]
    train_x_new = [row[:-1] for row in train_set_new]
    train_y_new = [row[-1] for row in train_set_new]

    test_set_new = dataset_new[:int(x / 100 * len(dataset_new))]
    test_x_new = [row[:-1] for row in test_set_new]
    test_y_new = [row[-1] for row in test_set_new]


    # train_x_2 = []
    # for i in train_set:
    #     row = [i[t] for t in range(len(i)) if t != least_importanc]
    ...
    classifier2.fit(scaler1.transform(train_x_new), train_y_new)

    classifier3 = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    classifier3.fit(scaler2.transform(train_x_new), train_y_new)

    acc1 = 0
    for i in range(len(test_x_new)):
        prediction = classifier2.predict(scaler1.transform([test_x_new[i]]))[0]
        if prediction == test_y_new[i]:
            acc1 += 1
    acc1 = acc1 / len(test_set)

    acc2 = 0
    for i in range(len(test_x_new)):
        prediction = classifier3.predict(scaler2.transform([test_x_new[i]]))[0]
        if prediction == test_y_new[i]:
            acc2 += 1
    acc2 /= len(test_set)

    print(f"Tocnost so StandardScaler: {acc1}")
    print(f"Tocnost so MinMaxScaler: {acc2}")