...
if __name__ == '__main__':
    ...
    class2 = [row for row in dataset if row[-1] == 2]

    train_set_1 = class0[:int(x1 * len(class0))] + class1[:int(x1 * len(class1))] + class2[:int(x1 * len(class2))]
    train_x_1 = [row[:-1] for row in train_set_1]
    train_y_1 = [row[-1] for row in train_set_1]

    test_set_1 = class0[int(x2 * len(class0)):] + class1[int(x2 * len(class1)):] + class2[int(x2 * len(class2)):]
    test_x_1 = [row[:-1] for row in test_set_1]
    test_y_1 = [row[-1] for row in test_set_1]

    train_set_2 = class0[int(x1 * len(class0)):int(x2 * len(class0))] + class1[int(x1 * len(class1)):int(x2 * len(class1))] + class2[int(x1 * len(class2)):int(x2 * len(class2))]
    train_x_2 = [row[:-1] for row in train_set_2]
    train_y_2 = [row[-1] for row in train_set_2]

    train_set_3 = class0[:int(x2 * len(class0))] + class1[:int(x2 * len(class1))] + class2[:int(x2 * len(class2))]
    ...
    classifier3 = RandomForestClassifier(n_estimators=3, random_state=0)
    classifier3.fit(train_x_3, train_y_3)

    acc = 0
    cnt = 0
    for i in range(len(test_set_1)):
        prediction1 = classifier1.predict([test_x_1[i]])[0]
        prediction2 = classifier2.predict([test_x_1[i]])[0]
        prediction3 = classifier3.predict([test_x_1[i]])[0]
        cnt = 0
        if prediction1 == test_y_1[i]:
            cnt += 1
        if prediction2 == test_y_1[i]:
            cnt += 1
        if prediction3 == test_y_1[i]:
            cnt += 1
        if cnt >= 2:
            acc += 1
