...
class Snake(Problem):
    def __init__(self, initial, goal, crveni):
        super().__init__(initial, goal)
        self.crveni = crveni

    def check_valid(self, snake_head, snake_body):
        if snake_head in self.crveni:
            return False
...
#[1:]
        ...
        if dir == 1:
            ...
            if self.check_valid(new_head, snake):
...
if __name__ == '__main__':
    ...
    for i in range(n):
        line = input().split(",")
        green_apple.append((int(line[0]), int(line[1])))
    m = int(input())
    for i in range(m):
        line = input().split(",")
        crveni_jabolki.append((int(line[0]), int(line[1])))

    rez = Snake((nasoka, zmija, tuple(green_apple)), None, crveni_jabolki)
    result = breadth_first_graph_search(rez)
