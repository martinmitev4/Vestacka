...
class Squares(Problem):
    ...
    #prvo kvadratce
        ...
        # gore
        new_pos = (pos_1[0], pos_1[1] + 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 1 gore'] = (new_pos, pos_2, pos_3, pos_4, pos_5)
            
        #levo
        new_pos = (pos_1[0] - 1, pos_1[1])
        if self.check_valid(new_pos) :
            succ['Pomesti kvadratche 1 levo'] = (new_pos, pos_2, pos_3, pos_4, pos_5)

        # desno
        new_pos = (pos_1[0] + 1, pos_1[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 1 desno'] = (new_pos, pos_2, pos_3, pos_4, pos_5)

        # dole
        new_pos = (pos_1[0], pos_1[1] - 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 1 dolu'] = (new_pos, pos_2, pos_3, pos_4, pos_5)

    # vtoro kvadratce

        # levo
        new_pos = (pos_2[0] - 1, pos_2[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 2 levo'] = (pos_1, new_pos, pos_3, pos_4, pos_5)

        # dole
        new_pos = (pos_2[0], pos_2[1] - 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 2 dolu'] = (pos_1, new_pos, pos_3, pos_4, pos_5)

        # desno
        new_pos = (pos_2[0] + 1, pos_2[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 2 desno'] = (pos_1, new_pos, pos_3, pos_4, pos_5)

        # gore
        new_pos = (pos_2[0], pos_2[1] + 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 2 gore'] = (pos_1, new_pos, pos_3, pos_4, pos_5)
    ...
    # treto kvadratce
        ...
        # levo
        new_pos = (pos_3[0] - 1, pos_3[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 3 levo'] = (pos_1, pos_2, new_pos, pos_4, pos_5)

        # desno
        new_pos = (pos_3[0] + 1, pos_3[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 3 desno'] = (pos_1, pos_2, new_pos, pos_4, pos_5)

        # gore
        new_pos = (pos_3[0], pos_3[1] + 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 3 gore'] = (pos_1, pos_2, new_pos, pos_4, pos_5)

        # dole
        new_pos = (pos_3[0], pos_3[1] - 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 3 dolu'] = (pos_1, pos_2, new_pos, pos_4, pos_5)

    # cetvrto kvadratce

        # levo
        new_pos = (pos_4[0] - 1, pos_4[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 4 levo'] = (pos_1, pos_2, pos_3, new_pos, pos_5)

        # desno
        new_pos = (pos_4[0] + 1, pos_4[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 4 desno'] = (pos_1, pos_2, pos_3, new_pos, pos_5)

        # gore
        new_pos = (pos_4[0], pos_4[1] + 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 4 gore'] = (pos_1, pos_2, pos_3, new_pos, pos_5)

        # dole
        new_pos = (pos_4[0], pos_4[1] - 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 4 dolu'] = (pos_1, pos_2, pos_3, new_pos, pos_5)

    # peto kvadratce

        # levo
        new_pos = (pos_5[0] - 1, pos_5[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 5 levo'] = (pos_1, pos_2, pos_3, pos_4, new_pos)

        # desno
        new_pos = (pos_5[0] + 1, pos_5[1])
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 5 desno'] = (pos_1, pos_2, pos_3, pos_4, new_pos)

        # gore
        new_pos = (pos_5[0], pos_5[1] + 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 5 gore'] = (pos_1, pos_2, pos_3, pos_4, new_pos)

        # dole
        new_pos = (pos_5[0], pos_5[1] - 1)
        if self.check_valid(new_pos):
            succ['Pomesti kvadratche 5 dolu'] = (pos_1, pos_2, pos_3, pos_4, new_pos)
