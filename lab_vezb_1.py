...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        ghost_x = state[0]
        ghost_y = state[1]
        if 0 <= ghost_x < n and 0 <= ghost_y < n and (ghost_x, ghost_y) not in walls:
            return True
        return False
    ...
    def h(self, node):
        ghost_x = node.state[0]
        ghost_y = node.state[1]
        goal_x = self.goal[0]
        goal_y = self.goal[0]
        manh = abs((goal_x - ghost_x) + (goal_y - ghost_y))/3
        return manh
