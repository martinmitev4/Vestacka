...
class Football(Problem):
    def __init__(self, opponets, initial, goal=None):
        super().__init__(initial, goal)
        self.opponets = opponets
        self.size = [8, 6]
    ...
    def check_valid(state, opponents):
        ...
        ball_pos = state[1]
        ball_opponets = (
            (2, 2), (2, 3), (2, 4), (3, 4), (4, 4), (4, 3), (4, 2), (3, 2), (4, 5), (5, 5), (5, 3), (6, 3), (6, 4),
            (6, 5))

        return man_pos[0] >= 0 and man_pos[0] < 8 and \
            man_pos[1] >= 0 and man_pos[1] < 6 and \
            ball_pos[0] >= 0 and ball_pos[0] < 8 and \
            ball_pos[1] >= 0 and ball_pos[
                1] < 6 and man_pos != ball_pos and ball_pos not in ball_opponets and man_pos not in opponents and ball_pos not in opponents
    ...
    def successor(self, state):
        ...
        if new_man == (ball_x, ball_y):
            new_ball = (ball_x, ball_y + 1)
        else:
            new_ball = (ball_x, ball_y)
        new_state = (new_man, new_ball)
        if self.check_valid(new_state, self.opponets):
            if new_ball == (ball_x, ball_y):
...
if __name__ == '__main__':
    ...
    fot = Football(opponents, (man_pos, ball_pos))

    # result = astar_search(fot)
    result = astar_search(fot)
