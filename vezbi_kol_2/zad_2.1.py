...
if __name__ == '__main__':
    ...
    col = int(input())
    entry = [float(en) for en in input().split(" ")]

    scaler = MinMaxScaler((-1, 1))
    scaler.fit(train_x)

    classifier = MLPClassifier(nev, activation='relu', max_iter=20, random_state=0, learning_rate_init=learning)
    classifier.fit(scaler.transform(train_x), train_y)

    acc = 0
    for i in range(len(test_set)):
        predict = classifier.predict(scaler.transform([test_x[i]]))[0]
        if predict == test_y[i]:
            acc += 1
    ...
    for i in range(len(train_set)):
        predict = classifier.predict(scaler.transform([train_x[i]]))[0]
        if predict == train_y[i]:
            acc_train += 1

    acc_train /= len(train_set)

    if acc_train < acc + acc*0.15:
        ...
        train_set_2 = new_data[:int(0.8 * len(new_data))]
        train_x_2 = [row[:-1] for row in train_set_2]
        train_y_2 = [row[-1] for row in train_set_2]

        test_set_2 = new_data[int(0.8 * len(new_data)):]
