...
class Disk(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(0, len(state)):
            pos = list(state)
            if i < len(pos) - 1 and pos[i] != 0 and pos[i + 1] == 0:
                tem = pos[i]
                pos[i] = pos[i + 1]
                pos[i + 1] = tem
                succesor['D1: Disk ' + str(tem)] = tuple(pos)

            if i < len(pos) - 2 and pos[i] != 0 and pos[i + 1] != 0 and pos[i + 2] == 0:
                tem = pos[i]
                pos[i] = pos[i + 2]
                pos[i + 2] = tem
                succesor['D2: Disk ' + str(tem)] = tuple(pos)

            if pos[i] != 0 and i - 1 >= 0 and pos[i - 1] == 0:
                tem = pos[i]
                pos[i] = pos[i - 1]
                pos[i - 1] = tem
                succesor['L1: Disk ' + str(tem)] = tuple(pos)

            if pos[i] != 0 and i - 2 >= 0 and pos[i - 1] != 0 and pos[i - 2] == 0:
                tem = pos[i]
                pos[i] = pos[i - 2]
                pos[i - 2] = tem
                succesor['L2: Disk ' + str(tem)] = tuple(pos)
    ...
    def h(self, node):
        ...
        #     check_state.append(node.state[len(node.state) - i - 1])

        for i in range(len(node.state)):
            if self.goal[i] != 0 and node.state[i] != self.goal[i]:
                cnt += 1
...
if __name__ == "__main__":
    ...
    for i in range(l):
        goal_state.append(initial_state[l - i - 1])

    problem = Disk(tuple(initial_state), tuple(goal_state))
