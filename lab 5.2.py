...
if __name__ == '__main__':
    ...
    max_tree = int(input())
    criteria = input()
    en = [element for element in input().split(' ')]
    ...
    clasifikator = RandomForestClassifier(criterion= criteria, random_state=0, n_estimators=max_tree)
    ...
    clasifikator.fit(train_x_2, train_y)

    tocnost = 0

    for i in range(len(test_x_2)):
        predicted_class = clasifikator.predict([test_x_2[i]])[0]
        true_class = test_y[i]
        if true_class == predicted_class:
            tocnost += 1

    tocnost = tocnost / len(test_x_2)
    print(f'Accuracy: {tocnost}')

    # for t in en:

    en_2 = [en[i] for i in range(len(en)) if i != col_index]

    nova_class = clasifikator.predict([en_2])[0]
    print(nova_class)
    verojatnosti = clasifikator.predict_proba([en_2])
    print(verojatnosti[0])