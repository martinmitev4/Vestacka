dataset = [
           [180.0, 23.6, 25.2, 27.9, 25.4, 14.0, 'Roach'],
...
if __name__ == '__main__':
    ...
    new_data = []
    for row in dataset:
        newrow = [row[i] for i in range(len(row)) if i != col_index]
    ...
    classifier = RandomForestClassifier(criterion=ent, n_estimators=n, random_state=0)
    ...
    en = [entry for entry in input().split(' ')]
    en = [en[i] for i in range(len(en)) if i != col_index]

    print(classifier.predict([en])[0])

    print(classifier.predict_proba([en])[0])
