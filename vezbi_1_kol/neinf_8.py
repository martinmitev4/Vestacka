...
class Paint(Problem):
    ...
    def smeni(self, state, i1, j1, n1):
        nov = list([list(item) for item in state])
        i1 = int(i1)
        j1 = int(j1)
        n1 = int(n1)
        # print(state)
        ...
        return tuple((tuple(item) for item in nov))
    ...
    def successor(self, state):
        ...
        for i in range(0, self.n):
            for j in range(0, self.n):
                new_state = self.smeni(state, i, j, self.n)
                sucessor['x: '+str(i)+', y: ' + str(j)] = new_state
...
if __name__ == "__main__":
    ...
    problem = Paint(n, tuple(matrix), tuple(goal_state))
