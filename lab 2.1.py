...
class Snake(Problem):
    ...
    def check_valid(self, head, body):
        if head[0] < 0 and head[0] > 9 and head[1] < 0 and head[1] > 9 and head in body:
            return False
        return True

    def move(self, nasoka, head, snake, zeleni):
        tmp = snake[0]
        new_snake = [head]
        for x in snake[:-1]:
            new_snake.append(tmp)
            tmp = x
        if head in zeleni:
            new_snake.append(tmp)
            return (nasoka, tuple(new_snake), tuple([apple for apple in zeleni
                                                     if apple != head]))
        return (nasoka, tuple(new_snake), zeleni)
    ...
    def successor(self, state):
        ...
        if dir == 1:
            ...
            if self.check_valid(new_head, snake[1:]):
                new_state = self.move(nasoka, new_head, snake, green)
                successors['ProdolzhiPravo'] = new_state
            # desno
            new_head = (snake[0][0], snake[0][1] - 1)
            if self.check_valid(new_head, snake[1:]):
                new_state = self.move(4, new_head, snake, green)
                successors['SvrtiDesno'] = new_state
            # levo
            new_head = (snake[0][0], snake[0][1] + 1)
            if self.check_valid(new_head, snake[1:]):
                new_state = self.move(2, new_head, snake, green)
        ...
        if dir == 2:
            ...
            new_head = (snake[0][0] - 1, snake[0][1])
            if self.check_valid(new_head, snake[1:]):
                new_state = self.move(3, new_head, snake, green)
                successors['SvrtiLevo'] = new_state
            # desno
            new_head = (snake[0][0] + 1, snake[0][1])
            if self.check_valid(new_head, snake[1:]):
                new_state = self.move(1, new_head, snake, green)
                successors['SvrtiDesno'] = new_state
        # nasoka 3
        # ****
        # **<*
    ...
    def h(self, node):
        ...
        head_y = node.state[1][0][1]
        green = node.state[2]
        lista = []

        for x in green:
            a = abs(head_x - x[0]) + abs(head_y - x[1])
            lista.append(a)
        ...
        for x in lista:
            minn = max(x, minn)
        # a = max(abs(x - x1) + abs(y - y1), abs(x - x2) + abs(y - y2))
        # (7, 2)  (7, 3)
...
if __name__ == "__main__":
    ...
    rez = Snake((nasoka, zmija, tuple(zeleni_jabolki)), None)
    result = astar_search(rez)
