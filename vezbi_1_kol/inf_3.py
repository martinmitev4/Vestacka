...
class Packman(Problem):
    def __init__(self, initial, goal=None):
        ...
        self.precki = (
            (0, 6), (0, 8), (0, 9), (1, 2), (1, 3), (1, 4), (1, 9), (2, 9), (3, 6), (3, 9), (4, 1), (4, 5), (4, 6),
            (4, 7),
            (5, 1), (5, 6), (6, 0), (6, 1), (6, 2), (6, 9), (8, 1), (8, 4), (8, 7), (8, 8), (9, 4), (9, 7), (9, 8))

    def izedi (self,dir, man, dots):
        new_dots = []
        if man in dots:
            for x in dots:
                if man != x:
                    new_dots.append(x)
            return (dir, man, tuple(new_dots))
        return (dir, man, dots)

    def check_valid(self, man):
        man_x = man[0]
        man_y = man[1]
        if man_x >= 0 and man_x <= 9 and man_y >= 0 and man_y <= 9 and man not in self.precki:
            return True
        return False
    ...
    def successor(self, state):
        ...
        if dire == 'istok':
            ...
            if self.check_valid(new_man):
                new_state = self.izedi(dire, new_man, dots)
                successors['ProdolzhiPravo'] = new_state
            # nazad
            new_man = (man[0] - 1, man[1])
            if self.check_valid(new_man):
                new_state = self.izedi('zapad', new_man, dots)
                successors['ProdolzhiNazad'] = new_state
            # levo
            new_man = (man[0], man[1] + 1)
            if self.check_valid(new_man):
                new_state = self.izedi('sever', new_man, dots)
                successors['SvrtiLevo'] = new_state
            # desno
            new_man = (man[0], man[1] - 1)
            if self.check_valid(new_man):
                new_state = self.izedi('jug', new_man, dots)
        ...
        if dire == 'zapad':
            ...
            new_man = (man[0] + 1, man[1])
            if self.check_valid(new_man):
                new_state = self.izedi('istok', new_man, dots)
                successors['ProdolzhiNazad'] = new_state
            # levo
            new_man = (man[0], man[1] - 1)
            if self.check_valid(new_man):
    ...
    def h(self, node):
        head_x = node.state[1][0]
        head_y = node.state[1][1]
        green = node.state[2]
        lista = []
...
if __name__ == "__main__":
    ...
    state = (direction, man, tuple(dots))
    sol = Packman(state, None)
    resenie = astar_search(sol)
